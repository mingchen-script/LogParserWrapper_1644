# Readme..
	...
	#
	# LogParserWrapper_1644.ps1 v0.6 11/16 (added BarCharts)
	#		Steps: 
...
$g_MaxImports = 99999                 # Max number of 1644 events to import into excel, Use this for quicker spot checks.
...
if ($g_LookBackDays -ne 0) { $g_StartTime = (Get-Date).AddDays(0-$g_LookBackDays) -f 'M/d/yyyy H:m:s tt' }
  ...
  $TotalSteps = ((Get-ChildItem -Path $ScriptDirectory -Filter '*.evtx').count)+9
    $Step=1
  $TimeStamp = "{0:yyyy-MM-dd_hh-mm-ss_tt}" -f (Get-Date)
  (Get-ChildItem -Path $ScriptDirectory -Filter '*.evtx').foreach({
      $InFile = $ScriptPath+'\'+$_
...
"@
    ...
    while ((get-service eventlog).status -ne 'Running') { 
      ...
      catch {  Write-Host 'Unable to start EventLog service, possible persmission issue, try restart script under admin prompt.' -BackgroundColor Red
        Start-Sleep 5 }
      }
...
#-----Combine CSV(s) into one for faster Excel import
  $InFiles = "$ScriptPath\$TimeStamp-Temp1644-*.csv"
  ...
  $Query = @"
    SELECT TOP $g_MaxImports 
      LDAPServer,TimeGenerated,ClientIP,ClientPort,StartingNode,Filter,SearchScope,AttributeSelection,ServerControls,VisitedEntries,ReturnedEntries,UsedIndexes,
      PagesReferenced,PagesReadFromDisk,PagesPreReadFromDisk,CleanPagesModified,DirtyPagesModified,SearchTimeMS
    INTO $OutFile1
    FROM $InFiles
"@
  Write-Progress -Activity "Generating $OutTitle report" -PercentComplete (($Step++/$TotalSteps)*100)
  ...
  $null = Get-ChildItem -Path $ScriptDirectory -Filter "$TimeStamp-Temp1644-*.csv" | Remove-Item
#-------Import to Excel
If (Test-Path $OutFile1) { 
  ...
  Write-Progress -Activity "Import to Excel $OutTitle report" -PercentComplete (($Step++/$TotalSteps)*100)
    ...
    $Sheet0 = $Excel.Workbooks[1].Worksheets[1]
      ...
      $Sheet0.Columns.Item(5).columnwidth = $Sheet0.Columns.Item(6).columnwidth = $Sheet0.Columns.Item(8).columnwidth = $Sheet0.Columns.Item(9).columnwidth = $Sheet0.Columns.Item(12).columnwidth = 70
      $Sheet0.Columns.Item('J').numberformat = $Sheet0.Columns.Item('K').numberformat = $Sheet0.Columns.Item('M').numberformat = $Sheet0.Columns.Item('N').numberformat = $Sheet0.Columns.Item('O').numberformat = $Sheet0.Columns.Item('p').numberformat = $Sheet0.Columns.Item('Q').numberformat = $Sheet0.Columns.Item('R').numberformat = "###,###,###,###,###"
    ...
    $Sheet1 = $Excel.Workbooks[1].Worksheets.add()
      ...
      Set-PivotField -PivotField $Sheet1.PivotTables("PivotTable1").PivotFields("LDAPServer") -Orientation $xlPageField

        Set-PivotField -PivotField $Sheet1.PivotTables("PivotTable1").PivotFields("StartingNode") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet1.PivotTables("PivotTable1").PivotFields("Filter") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet1.PivotTables("PivotTable1").PivotFields("ClientIP") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet1.PivotTables("PivotTable1").PivotFields("TimeGenerated") -Orientation $xlRowField -Group 4
    ...
    $Sheet2 = $Excel.Workbooks[1].Worksheets.add()
      ...
      Set-PivotField -PivotField $Sheet2.PivotTables("PivotTable2").PivotFields("LDAPServer") -Orientation $xlPageField

        Set-PivotField -PivotField $Sheet2.PivotTables("PivotTable2").PivotFields("ClientIP") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet2.PivotTables("PivotTable2").PivotFields("Filter") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet2.PivotTables("PivotTable2").PivotFields("TimeGenerated") -Orientation $xlRowField -Group 3
    ...
    $Sheet3 = $Excel.Workbooks[1].Worksheets.add()
      ...
      Set-PivotField -PivotField $Sheet3.PivotTables("PivotTable3").PivotFields("LDAPServer") -Orientation $xlPageField

        Set-PivotField -PivotField $Sheet3.PivotTables("PivotTable3").PivotFields("Filter") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet3.PivotTables("PivotTable3").PivotFields("ClientIP") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet3.PivotTables("PivotTable3").PivotFields("TimeGenerated") -Orientation $xlRowField -Group 3
    ...
    $Sheet4 = $Excel.Workbooks[1].Worksheets.add()
      ...
      Set-PivotField -PivotField $Sheet4.PivotTables("PivotTable4").PivotFields("LDAPServer") -Orientation $xlPageField

        Set-PivotField -PivotField $Sheet4.PivotTables("PivotTable4").PivotFields("ClientIP") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet4.PivotTables("PivotTable4").PivotFields("Filter") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet4.PivotTables("PivotTable4").PivotFields("TimeGenerated") -Orientation $xlRowField -Group 3
    ...
    $Sheet5 = $Excel.Workbooks[1].Worksheets.add()
      ...
      Set-PivotField -PivotField $Sheet5.PivotTables("PivotTable5").PivotFields("LDAPServer") -Orientation $xlPageField

        Set-PivotField -PivotField $Sheet5.PivotTables("PivotTable5").PivotFields("Filter") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet5.PivotTables("PivotTable5").PivotFields("ClientIP") -Orientation $xlRowField
        Set-PivotField -PivotField $Sheet5.PivotTables("PivotTable5").PivotFields("TimeGenerated") -Orientation $xlRowField -Group 3
    ...
    $WorkSheetNames = New-Object System.Collections.ArrayList
      foreach ($mcWorkSheet in $Excel.Workbooks[1].Worksheets) { $null = $WorkSheetNames.add($mcWorkSheet.Name) }
      $Tmp = $WorkSheetNames.Sort() | Out-Null
        For ($i=0; $i -lt $WorkSheetNames.Count-1; $i++){ #Sort name. 
          $Tmp = $WorkSheetNames[$i]
          $Before = $Excel.Workbooks[1].Worksheets.Item($Tmp)
          $After = $Excel.Workbooks[1].Worksheets.Item($i+1)
          $Before.Move($After)
        }
      $Sheet1.Activate()
    #----Save and clean up
